...
export default function Dashboard() {
    ...
    useEffect(() => {
        ...
        const init = async () => {
            const [subjectList, rawSessions] = await Promise.all([
                fetchSubjects(),
                fetchSessions(),
            ]);
    ...
    sessions.forEach((session) => {
        subjectTimeMap[session.subjectId] =
            (subjectTimeMap[session.subjectId] || 0) + (session.timeAllotted || 0);
    });
                            ...
                            <Card key={subject.id} className="flex items-center justify-between px-4 py-2">
                                <div>
                                    ...
                                    <p className="text-sm text-muted-foreground">
                                        ⏱ {formatDuration(subjectTime)} total
                                    </p>
